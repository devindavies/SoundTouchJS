...
export default class PitchShifter {
	...
	duration: number;
	sampleRate: number;
	listeners: { name: string; cb: (event: Event) => void }[];
	...
	set percentagePlayed(perc) {
		this._filter.sourcePosition = perc * this.duration * this.sampleRate;
	...
	off(eventName = null) {
		...
		for (const listener of listeners) {
			this._node.removeEventListener(listener.name, (event) =>
				listener.cb(event),
			);
		}
	}
