...
export default class Stretch extends AbstractFifoSamplePipe {
	_quickSeek: boolean;
	midBufferDirty: boolean;
	midBuffer: Float32Array | null;
	overlapLength: number;
	autoSeqSetting: boolean;
	autoSeekSetting: boolean;
	_tempo: number;
	sampleRate: number;
	overlapMs: number;
	sequenceMs: number;
	 ...
	 */
	setParameters(sampleRate: number, sequenceMs, seekWindowMs, overlapMs) {
	...
	preCalculateCorrelationReferenceStereo() {
		...
		if (!this.midBuffer) {
			throw new Error("midBuffer is null");
		}
	...
	calculateCrossCorrelationStereo(
		mixingPositionParam: number,
		compare: Float32Array,
	) {
		const mixing = this._inputBuffer.vector;
		const mixingPosition = mixingPositionParam + this._inputBuffer.startIndex;
	 ...
	 */
	overlapStereo(inputPositionParam: number) {
		const input = this._inputBuffer.vector;
		const inputPosition = inputPositionParam + this._inputBuffer.startIndex;
